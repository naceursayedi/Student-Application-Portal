...
@pytest.mark.title("Testing Degree Course Application  - Create Degree Course Application")
...
def test_create_degree_course_application_admin():
    ...
    ApplicantUserID = driver.find_element(
        By.ID, 'CreateDegreeCourseApplicationEditUserID')
    ApplicantUserID.send_keys(
        restapi_test_data["degreeCourseApplication"]["application_admin"]["applicantUserID"])

    ApplicantUserID = driver.find_element(
        By.ID, 'CreateDegreeCourseApplicationEditTargetPeriodYear')
    ApplicantUserID.send_keys(
        restapi_test_data["degreeCourseApplication"]["application_admin"]["targetPeriodYear"])
    ...
    select_object.select_by_value(
        restapi_test_data["degreeCourseApplication"]["application_admin"]["targetPeriodShortName"])
    ...
    assert restapi_test_data["degreeCourseApplication"]["application_admin"]["applicantUserID"] + ": " + \
        restapi_test_data["degreeCourseApplication"]["application_admin"]["targetPeriodShortName"] + str(restapi_test_data["degreeCourseApplication"]["application_admin"]["targetPeriodYear"]) in CreatedDegreeCourseApplicationItem.get_attribute(
            'innerHTML'), "The created course Application should be appeared directly after creation"


@pytest.mark.title("Testing Degree Course Application  - Delete Degree Course Application as Admin")
@pytest.mark.description("This test checks the create process and if the created Degree Course Application appeared after creation")
def test_delete_degree_course_application_admin():
